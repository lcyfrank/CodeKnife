...
def _check_storage_type(method, method_hub, method_cache, recursive_set=set([]), verbose=False):
            ...
            if called_method is not None:
                if class_method_cache is not None and method_name in class_method_cache:  # Already
                    ...
                    if called_method in recursive_set:
                        ...
                        u, k, s, c = _check_storage_type(called_method, method_hub, method_cache, recursive_set.copy(), verbose)
                        k_user_defaults |= u
                        k_keyed_archive |= k
                        k_sqlite |= s
                        k_core_data |= c
...
def check_storage_type(method_hub, verbose=False):
    ...
    for class_key in method_hub.method_insns:  # 这样遍历字典速度比较快
        ...
        for method in class_methods:  # 遍历这个类的所有方法
            if class_method_cache is not None and method.method_name in class_method_cache:  # 这个方法之前分析过了
                ...
                u, k, s, c = _check_storage_type(method, method_hub, method_cache, verbose=verbose)
